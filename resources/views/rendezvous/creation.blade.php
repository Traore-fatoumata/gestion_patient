...
<head>
    ...
    <title>Ajouter un Rendez-vous - Back-Office</title>
...
<body class="bg-gray-100 font-sans">
    ...
    <section class="container mx-auto py-12">
        <h1 class="text-3xl font-bold mb-6">Ajouter un Rendez-vous</h1>
        @if($errors->any())
        ...
        @if($patients->isEmpty() || $medecins->isEmpty())
            <div class="bg-blue-300 border-4-4 border-blue-300 text-white-700 p-4 mb-9 " role="alert">
                Veuillez remplir ces champs pour la création un rendez-vous.
            </div>
        @endif
            ...
            <div class="mb-4">
                ...
                <select name="medecin_id" id="medecin_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    @foreach($medecins as $medecin)
                        <option value="{{ $medecin->id }}">{{ $medecin->nom }} {{ $medecin->prenom }} ({{ optional($medecin->specialite)->nom ?? 'Non spécifié' }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="date_heure" class="block text-sm font-medium text-gray-700">Date/heure</label>
                <input type="datetime-local" name="date_heure" id="date_heure" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div class="mb-4">
                <label for="raison" class="block text-sm font-medium text-gray-700">Motif</label>
                <textarea name="raison" id="raison" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"></textarea>
            </div>
            <div class="mb-4">
