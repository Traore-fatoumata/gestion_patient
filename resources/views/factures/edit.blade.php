...
<head>
    ...
    <title>Modifier la Facture - Back-Office</title>
...
<body class="bg-gray-100 font-sans">
    ...
    <section class="container mx-auto py-12">
        <h1 class="text-3xl font-bold mb-6">Modifier la Facture #{{ $facture->numero }}</h1>
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('factures.update', $facture) }}" method="POST" class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="numero" class="block text-gray-700 font-semibold mb-2">Numéro de facture</label>
                <input type="text" name="numero" id="numero" value="{{ old('numero', $facture->numero) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
                ...
                <select name="patient_id" id="patient_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    ...
                    @foreach ($patients as $patient)
                        <option value="{{ $patient->id }}" {{ old('patient_id', $facture->patient_id) == $patient->id ? 'selected' : '' }}>{{ $patient->nom }} {{ $patient->prenom }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="rendez_vous_id" id="rendez_vous_id" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    ...
                    @foreach ($rendezVous as $rdv)
                        <option value="{{ $rdv->id }}" {{ old('rendez_vous_id', $facture->rendez_vous_id) == $rdv->id ? 'selected' : '' }}>{{ $rdv->date_heure }} - {{ $rdv->patient->nom }} ({{ $rdv->medecin->nom }})</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label for="montant" class="block text-gray-700 font-semibold mb-2">Montant (€)</label>
                <input type="number" name="montant" id="montant" step="0.01" value="{{ old('montant', $facture->montant) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
                <label for="date_emission" class="block text-gray-700 font-semibold mb-2">Date d’émission</label>
                <input type="date" name="date_emission" id="date_emission" value="{{ old('date_emission', $facture->date_emission->format('Y-m-d')) }}" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $facture->description) }}</textarea>
            </div>
            <div class="mt-4">
